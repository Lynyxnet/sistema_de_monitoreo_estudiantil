...
if (!empty($materia) && !empty($materia) && !empty($_POST['dias'])) { //Si es diferente de vacio entra en la condicional
  ...
  //NOTA IMPORTANTE: Para que los alumnos puedan ser insertados en la base de datos, el nombre de la materia debe ser cambiado en el excel dependiendo de la clase creada anteriormente
  //Solo ve al archivo excele y cambia en la celda "asignatura" el nombre de la materia ya creada
  ...
  if ($archivo_extension == "xlsx" || $archivo_extension == "xls") {
    ...
    if (move_uploaded_file($archivo_tmp, $directorio_destino . $nuevo_archivo)) { //El nombre del archivo temporal de el archivo en el cual el archivo subido fue almacenado en el server
      ...
      if ($success == 4) {
        $mensajes[] = "Las matriculas ya estan relacionadas";
        $mensajes[] = "RERCUERDA: Cambiar le nombre de la asignatura en el excel para agregar alumnos en tal materia";
      }
      if ($success == 5) {
        $mensajes[] = "El alumno y materia se relaciono correctamente";
      }
